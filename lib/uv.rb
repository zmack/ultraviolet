...
module Uv
  ...
  def self.get_render_processor(render_style, options)
    output = options.delete(:output) || 'xhtml'

    renderer = get_renderer(render_style, output)

    render_options = YAML.load(File.open(renderer))
    render_processor = RenderProcessor.new(render_options, options)
  end

  def self.get_renderer(render_style, output)
